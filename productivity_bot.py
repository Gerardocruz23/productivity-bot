...
from telegram.ext import Application, CommandHandler, CallbackQueryHandler, MessageHandler, f
import sqlite3
...
"""
if update.message:
await update.message.reply_text(welcome_text, reply_markup=reply_markup, parse_mode='
else:
await update.callback_query.message.reply_text(welcome_text, reply_markup=reply_marku
# ============================================
# TAREAS
# ============================================
...
await query.edit_message_text(
" Escribe el nombre de tu nuevo habito:\n\n(Escribe /cancelar para cancelar)"
)
...
async def start_pomodoro(update: Update, context: ContextTypes.DEFAULT_TYPE, duration: """Iniciar sesion Pomodoro"""
query = update.callback_query
await query.answer()
int):
...
plt.close()
# Enviar grafico
await query.message.reply_photo(photo=buf, caption=" *Tus estadisticas de productividad
...
(habit_id, user_id))
if c.fetchone():
await query.answer(" Ya registraste este habito hoy", show_alert=True)
else:
c.execute('INSERT INTO habit_logs (habit_id, user_id) VALUES (?, ?)', (habit_id, user
conn.commit()
...
keyboard = [[InlineKeyboardButton("« Volver", callback_data='back_main')]]
reply_markup = InlineKeyboardMarkup(keyboard)
await update.callback_query.edit_message_text(help_text, reply_markup=reply_markup, p
else:
...
del context.user_data['waiting_for']
await update.message.reply_text(" Operacion cancelada. Usa /start para volver al me
else:
await update.message.reply_text("No hay nada que cancelar. Usa /start para ver el men
# ============================================
# MAIN
# ============================================
def main():
